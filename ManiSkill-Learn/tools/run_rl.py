...
def parse_args():
    ...
    dir_agent.add_argument('--no-agent-type', default=False,
                           action='store_true', help='no agent type')
    ...
    parser.add_argument(
        '--clean-up', help='Clean up the work_dir', action='store_true')

    # If we use evaluation mode
    parser.add_argument(
        '--evaluation', help='Use evaluation mode', action='store_true')
    parser.add_argument(
        '--test-name', help='The name of the folder to save the test result', default=None)
    ...
    group_gpus.add_argument('--num-gpus', default=None,
                            type=int, help='number of gpus to use')
    group_gpus.add_argument('--gpu-ids', default=None,
                            type=int, nargs='+', help='ids of gpus to use')
...
def main_mfrl_brl(cfg, args, rollout, evaluator, logger):
    ...
    if len(args.gpu_ids) > 1:
        ...
        agent.to_data_parallel(device_ids=args.gpu_ids,
...
def main():
    ...
    if args.auto_resume or (args.evaluation and cfg.get('resume_from', None) is None):
        ...
        model_names = list(
            glob.glob(osp.join(cfg.work_dir, 'models', '*.ckpt')))
        latest_index = -1
        latest_name = None
    ...
    from mani_skill_learn.methods.builder import MBRL, MFRL, BRL
